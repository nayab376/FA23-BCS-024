...
export default function PostAdModal({ onClose, onSubmit, defaultCategory }: PostAdModalProps) {
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [price, setPrice] = useState('');
  const [category, setCategory] = useState(
    defaultCategory && CATEGORY_NAMES.includes(defaultCategory as never)
      ? defaultCategory
      : CATEGORY_NAMES[0],
  );
